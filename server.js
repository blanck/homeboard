...
if (config.netatmo.client_id) {
  ...
  netatmoapi.on('error', function(error) {
...
if (config.newsapi.key) {
  try {
    ...
    if (config.newsapi.fallbackKey) {
      try {
        newsapi = new NewsAPI(config.newsapi.fallbackKey)
      } catch (err) {
        console.log(err)
      }
    }
  ...
  sonosDiscover
    ...
    .then((groups) => {
      groups.forEach((group) => {
        if (
          group.Name.substring(0, config.sonos.group.length) == config.sonos.group ||
          group.Name.includes(' + ')
        ) {
...
const discoverTibber = () => {
  console.log('Discover Tibber devices')
  tibberQuery2
    .query(
      'fragment SLFragment on SettingLayoutItem {type,title,description,valueText,imgUrl,isUpdated,isEnabled,settingKey,settingKeyForIsHidden}{me{home(id:"' +
        config.tibber1.homeId +
        '"){powerups{items{type,pairableDevice{deviceType,isPaired,isPairable,settingsLayout{...SLFragment,childItems{...SLFragment}},pairedDevices{deviceId}}}}}}}'
    )
    .then((res) => {
      // console.log('res', JSON.stringify(res, null, 2))
      if (res.me && res.me.home && res.me.home.powerups && res.me.home.powerups.items) {
        for (const item of res.me.home.powerups.items) {
          if (item.pairableDevice && item.pairableDevice.isPaired) {
            if (
              item.pairableDevice.deviceType.includes('heat pump') &&
              item.pairableDevice.pairedDevices.length > 0
            ) {
              config.tibber2.thermostat =
                item.pairableDevice.deviceType +
                '#' +
                item.pairableDevice.pairedDevices[0].deviceId +
                '#90227'
            }
            if (
              item.pairableDevice.deviceType.includes('inverter device') &&
              item.pairableDevice.pairedDevices.length > 0
            ) {
            ...
            if (
              item.pairableDevice.deviceType.includes('tibber pulse') &&
              item.pairableDevice.pairedDevices.length > 0
            ) {
            ...
            if (
              item.pairableDevice.deviceType.includes('tibber homevolt') &&
              item.pairableDevice.pairedDevices.length > 0
            ) {
              config.tibber2.homevolt = item.pairableDevice.pairedDevices[0].deviceId
            }
          }
        }
...
var initTibberFeed = function() {
  if (config.tibber1.active) {
    const tibberQuery = new Tibber.TibberQuery(config.tibber1)
    const tibberFeed = new Tibber.TibberFeed(tibberQuery, 5000)

    //console.log(tibberFeed)
...
io.on('connection', function(socket) {
  ...
  socket.on('tibber', function(mode) {
    if (tibberQuery) {
      const query =
        'query {viewer { homes {      currentSubscription{        id validFrom validTo status priceInfo{ current{ total energy tax startsAt currency level } today { total startsAt } tomorrow { total startsAt }}}}} }'
  ...
  socket.on('tibber2', function(mode) {
    if (tibberQuery2) {
      if (config.tibber2.thermostat || config.tibber2.inverter || config.tibber2.homevolt) {
        let query = '{me {home(id:"' + config.tibber1.homeId + '") {    '
        if (config.tibber2.thermostat) {
          query +=
            'thermostat(id:"' +
            config.tibber2.thermostat +
            '") { state{ comfortTemperature } temperatureSensor { measurement { value } } }    '
        }
        ...
        // if (config.tibber2.homevolt) {
        //   query +=
        //     'homevolt(id:"' +
        //     config.tibber2.homevolt +
        //     '") {bubble {  value  percent} }  homevoltProduction (id:"' +
        //     config.tibber2.production +
        //     '"){keyFigures {  valueText  unitText  description} }    '
        // }
        query += '}}}'
        console.log('tibber2 query', query)
  ...
  socket.on('tibber3', function(mode) {
    if (tibberQuery2) {
      ...
      // console.log('tibber3 query', query)
      tibberQuery2.query(query).then((res) => {
        // console.log('res', JSON.stringify(res, null, 2))
        if (res.me) {
...
var getWeatherToken = function(callback) {
  ...
  return request('https://auth.netatmo.com/weathermap/token', (err, res, body) => {
    if (err) {
      return console.log(err)
    }
    if (body.indexOf('body') > -1) {
      let jsonResponse = JSON.parse(body)
      let access_token = jsonResponse.body
